...
function MiniProposal({ proposal, index }: { proposal: LOIProposal; index: number }) {
  ...
  return (
    <div className={`rounded-2xl border p-3 text-xs ${isBuyer ? "bg-sky/50 border-sky" : "bg-lime/30 border-lime/50"}`}>
      ...
      {proposal.notes && (
        <>
          ...
          {expanded && (
            <div className="mt-1.5 text-muted-foreground leading-relaxed bg-card rounded-xl p-2 border border-border">
      ...
      {proposal.status === "accepted" && (
        <Badge className="mt-1.5 bg-lime text-foreground text-xs border-lime">Accepted</Badge>
      )}
    </div>
  )
}
      ...
      <div className="border-b border-border sticky top-0 z-10 bg-background/95 backdrop-blur-sm">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <Badge className="bg-amber-500/20 text-amber-600 border-amber-500/30 text-xs ml-2 rounded-full">Judges Only</Badge>
          </div>
          <div className="flex items-center gap-3">
            {state.status === "running" && (
              <Badge className="bg-sky/50 text-foreground border-sky gap-1 animate-pulse rounded-full">
                <div className="w-1.5 h-1.5 rounded-full bg-foreground" />
                Live Negotiation
              </Badge>
            )}
            {isAgreed && <Badge className="bg-lime/50 text-foreground border-lime rounded-full">Deal Reached!</Badge>}
      ...
      <div className="max-w-7xl mx-auto px-6 py-6">
        {state.status === "idle" && (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            <div className="w-20 h-20 rounded-full bg-lime flex items-center justify-center mb-6 shadow-2xl shadow-lime/20">
            ...
            <div className="grid grid-cols-3 gap-4 max-w-2xl mb-8 text-sm">
              <div className="bg-lime/30 border border-lime/50 rounded-2xl p-4">
                ...
                <div className="text-foreground/70 text-xs">Floor: $2.8M</div>
              </div>
              ...
              <div className="bg-sky/50 border border-sky rounded-2xl p-4">
                ...
                <div className="text-foreground/70 text-xs">Max: $3.2M</div>
              </div>
            </div>
            <Button
              onClick={() => start(true)}
        ...
        {state.status !== "idle" && (
          <div className="grid grid-cols-5 gap-4">
            ...
            <div className="col-span-2">
              <div className="bg-lime/30 border border-lime/50 rounded-2xl p-4 mb-3">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <Badge className="bg-lime text-foreground border-lime text-xs rounded-full">CloudTrack Pro</Badge>
                </div>
              ...
              <div className="sticky top-24 space-y-4">
                ...
                {gap !== null && (
                  <div className="bg-card border border-border rounded-2xl p-4 text-center shadow-sm">
                    ...
                    <div className={`text-xl font-bold ${gap <= 0 ? "text-foreground" : gap < 300_000 ? "text-amber-500" : "text-foreground"}`}>
                  ...
                  <div className="bg-card border border-border rounded-2xl p-3 shadow-sm">
                    ...
                    <ResponsiveContainer width="100%" height={100}>
                      <LineChart data={state.convergenceHistory}>
                        ...
                        <Tooltip formatter={(v) => [fmt(Number(v ?? 0))]} contentStyle={{ background: "#fff", border: "1px solid #e5e5e5", fontSize: 11, borderRadius: 8 }} />
                        <Line type="monotone" dataKey="sellerPrice" stroke="#d4e157" strokeWidth={2} dot={{ r: 2 }} name="Seller" />
                        <Line type="monotone" dataKey="buyerPrice" stroke="#90caf9" strokeWidth={2} dot={{ r: 2 }} name="Buyer" />
                      </LineChart>
                    </ResponsiveContainer>
                ...
                {state.status === "running" && (
                  <div className="bg-card border border-border rounded-2xl p-3 text-center shadow-sm">
                    <div className="flex gap-1 justify-center">
                      {[0, 150, 300].map(d => (
                        <div key={d} className="w-2 h-2 rounded-full bg-foreground animate-bounce" style={{ animationDelay: `${d}ms` }} />
                      ))}
                    </div>
                ...
                {isAgreed && (
                  <div className="bg-lime/30 border border-lime/50 rounded-2xl p-4 text-center">
                ...
                {state.status === "error" && (
                  <div className="bg-red-500/10 border border-red-500/30 rounded-2xl p-3 text-xs text-red-600 text-center">
                    <AlertCircle className="h-4 w-4 mx-auto mb-1" />
                    {state.error || "Error occurred"}
                    <Button size="sm" variant="ghost" onClick={reset} className="mt-2 text-xs text-red-600 hover:text-red-500 w-full">
                      Retry
                    </Button>
                  </div>
                )}
            ...
            {/* Buyer column */}
            <div className="col-span-2">
              <div className="bg-sky/50 border border-sky rounded-2xl p-4 mb-3">
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-2">
                  ...
                  <Badge className="bg-sky text-foreground border-sky text-xs rounded-full">Apex Growth</Badge>
                </div>
